...
if ! command -v uv >/dev/null 2>&1; then
  echo "uv is not installed. Install it first (see https://docs.astral.sh/uv/)" >&2
  exit 1
fi
...
if [[ -z "${EXTENSION_ID}" ]]; then
  echo "Extension ID cannot be empty. Usage: ./scripts/install_linux_chrome.sh <extension_id> [--chromium]" >&2
  exit 1
fi
